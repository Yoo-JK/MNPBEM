%% 04_bemstat_layer / sphere — MATLAB
%  20nm Au sphere, 1nm above glass substrate (eps=2.25)
%  Normal incidence (theta=0) + Oblique (theta=45, TM)
...
title(sprintf('MATLAB normal sphere (t=%.3fs)', t_normal));
...
xlabel('Wavelength (nm)'); ylabel('Cross section (nm^2)');
title(sprintf('MATLAB oblique sphere (t=%.3fs)', t_oblique));
legend('ext','sca','Location','best'); grid on;
saveas(fig, fullfile(fig_dir,'oblique_matlab.png')); close(fig);

fprintf('[info] MATLAB 04_bemstat_layer/sphere done.\n');
