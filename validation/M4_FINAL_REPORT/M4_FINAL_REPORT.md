...
2. **Python (CPU) > MATLAB on every demo** — **10/13** demos faster on Python CPU (median 3.56× speedup)
   - MATLAB timing은 sequential하게 백그라운드에서 수집되며, 더 진행될수록 비교 demo 수가 증가합니다.
   - Python GPU vs MATLAB: **13/13** demos faster on Python+GPU
3. ✅ **MATLAB == Python (1e-12)** — perf=71, OK=1, warn=0, BAD=0 (of 72 demos)

## Timing summary

- Python CPU vs MATLAB: 10/13 demos Python is faster
- Python GPU vs MATLAB: 13/13 demos Python+GPU is faster
